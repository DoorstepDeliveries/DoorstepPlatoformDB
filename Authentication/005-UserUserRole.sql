...
CREATE TABLE [Authentication].[UserUserRole]
(
  ...
  CONSTRAINT [FK_UserUserRole_UserRole] FOREIGN KEY ([RoleId])
    REFERENCES [Authentication].[UserRole] ([Id]) ON DELETE CASCADE ON UPDATE CASCADE
);

GO
CREATE NONCLUSTERED INDEX [IX_UserUserRole_UserId]
    ON [Authentication].[UserUserRole] ([UserId] ASC);

GO
CREATE NONCLUSTERED INDEX [IX_UserUserRole_RoleId]
    ON [Authentication].[UserUserRole] ([RoleId] ASC);
